...
--
-- Table structure for table `prior_auth'
--

-- Using back-ticks (`) to quote identifiers is non-standard.
-- "IF EXISTS" is non-standard.
-- "CASCADE" is a preferable alternative to "RESTRICT" that may not be available (ISO 9075-2:200x feature F032)
DROP TABLE IF EXISTS `prior_auth` RESTRICT;

CREATE TABLE `prior_auth` (
-- INTEGER is standard, but it's range is implementation-defined, so it might not be big enough.
-- DECIMAL(9) would be fine, but MySQL 5.0 doesn't appear to support AUTO_INCREMENT on DECIMAL columns.
-- AUTO_INCREMENT is non-standard.
-- GENERATED BY DEFAULT AS IDENTITY ( START WITH 1 ) is a standard form of AUTO_INCREMENT if
--  your databse supports (ISO 9075-2:200x feature T147).
	`pa_id` INTEGER PRIMARY KEY AUTO_INCREMENT,
-- `bugint(20)` is non-standard, but we are matching the referenced column
-- `ON DELETE SET NULL` would be good for databases with support (ISO 9075-2:200x feature F191)
-- `ON UPDATE CASCADE` would be good for database with support (ISO 9075-2:200x feature F701)
	`pa_patient` bigint(20) NOT NULL REFERENCES `patient_data` ( `id` )  ,
	`pa_number` VARCHAR(31) NOT NULL,
	`pa_service` VARCHAR(31) NOT NULL,
-- SMALLINT is allowed to vary too much between implementations
	`pa_count` DECIMAL(4) NOT NULL,
	`pa_begin` DATE NOT NULL,
	`pa_end` DATE
-- ENGINE=MyISAM is non-standard
) ENGINE=MyISAM;
-- --------------------------------------------------------
