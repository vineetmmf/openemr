...
<body class="body_top">
<span class="title"><?php xl('Patient Reminder Batch Job','e')?></span>
<table>
 <tr>
  <td class='text' align='left' colspan="3"><br>
    <span class="text"><?php xl('The patient reminders have been processed according their scheduled dates.','e')?></span><br><br>
    <span class="text"><?php xl('Email delivery is immediate, while automated VOIP is sent to the service provider for further processing.','e')?></span><br><br>
    <input type="button" value="<?php xl('Close','e'); ?>" onClick="window.close()"><br><br><br>
  </td>
 </tr>
</table>
<br><br>
...
while ($myenrollrow = sqlFetchArray($enrollresult)) {
	$action_date = explode(",", $myenrollrow['action_date']);
	$action_completed = explode(",", $myenrollrow['action_completed']);
	$actionsql = "select * from health_plan_actions where plan_id = '".$myenrollrow['plan_id']."' order by action_id";
	$actionresult = sqlStatement($actionsql);
	$count = 0;
	$reminder_recreate = "";
	while ($myactionrow = sqlFetchArray($actionresult)) {
		if ($myactionrow['subactions'] != "" && $myactionrow['subactions'] != NULL) {
			...
			{
				$action_frequency = $myactionrow['frequency'];
	...
	if ($reminder_recreate == "YES")
	{
		...
		sqlQuery("delete from patient_reminders WHERE patient_id = '".$myenrollrow['patient_id']."' AND plan_id = '".$myenrollrow['plan_id']."' AND enroll_id='".$myenrollrow['enroll_id']."'");
...
if ($GLOBALS['clinical_decision_rules_and_patient_reminders']) {
	...
	$sql = "select reminder_id, reminder_name, patient_id, enroll_id, action_id, reminder_content, scheduled_date, voice_status, email_status from patient_reminders where scheduled_date = '".date("Y-m-d")."' and scheduled_date != '0000-00-00' and scheduled_date IS NOT NULL";
	$result = sqlStatement($sql);
	while ($myrow = sqlFetchArray($result))
	{
		// Select the action completions status based on enrollment ID.
		$enrollrow = sqlQuery("select action_completed from health_plan_enrollment where enroll_id='".$myrow['enroll_id']."'");
		...
		if (!in_array($myrow['action_id']."-YES", $action_completed)) {
			...
			$sql = "select fname, lname, email, phone_cell, hipaa_voice, hipaa_allowemail from patient_data where pid='".$myrow['patient_id']."'";
			$patientresult = sqlStatement($sql);
			if ($mypatientrow = sqlFetchArray($patientresult))
			{
				$patientfname = $mypatientrow['fname'];
				$patientlname = $mypatientrow['lname'];
				$patientemail = $mypatientrow['email'];
				$patientphone = $mypatientrow['phone_cell'];
				$hipaa_voice = $mypatientrow['hipaa_voice'];
				$hipaa_allowemail = $mypatientrow['hipaa_allowemail'];
			}
			// Email to patient if Allow Email field in Demographics is set to Yes.
			if ($hipaa_allowemail == "YES" and $myrow['email_status'] != "sent")
			{
				$mail = new MyMailer();
				...
				$mail->Body = $myrow['reminder_content'];
				...
				if(!$mail->Send()) {
					...
					sqlQuery("update patient_reminders set email_status='failed' where reminder_id='".$myrow['reminder_id']."'");
				}
				else
				{
					//echo "Email sent successfully...<br><br>";
					sqlQuery("update patient_reminders set email_status='sent' where reminder_id='".$myrow['reminder_id']."'");
				}
			}
			...
			if ($hipaa_voice == "YES" and $myrow['voice_status'] == "pending")
			{
				...
				$data = array(
					...
					"type" => "reminder",
					"timeZone" => date('P'),
					"greeting" => $myrow['reminder_content']
				);
				...
				if($response->IsError)
				{
					...
					sqlQuery("update patient_reminders set voice_status='failed' where reminder_id='".$myrow['reminder_id']."'");
				}
				else
				{
					// echo "Phone call done successfully...<br><br>";
					sqlQuery("update patient_reminders set voice_status='sent' where reminder_id='".$myrow['reminder_id']."'");
				}
			}
		}
	}
