...
function rx_parser($str) {
	...
	//	$fields = preg_split('/[\.\s+]/',$string);
		...
		if (preg_match("/^([rdpP])(\d*)/",$fields[0],$matches)) {
			$drugname1 = 'ibuprofen';
			$drugname2 = 'ibuprofen ';
			if ($matches[1] == 'd') {$drugname1 = 'naproxyn';$drugname2 = 'naproxyn ';}
			if ($matches[1] == 'p') {$drugname1 = 'hydrocodone';$drugname2 = 'hydrocodone 10/325 ';}
			if ($matches[1] == 'P') {$drugname1 = 'hydrocodone';$drugname2 = 'hydrocodone 10/650 ';}
			array_shift($fields);
		...
		if (preg_match("/^([vx])(\d*)/",$fields[0],$matches)) {
			$drugname1 = 'diphenhydramine';
			$drugname2 = 'diphenhydramine ';
			if ($matches[1] == 'v') {$drugname1 = 'diphenhydramine';$drugname2 = 'diphenhydramine ';}
			array_shift($fields);
			if ($matches[2]) {
				$fields[0] = '2'; 
		...
		if (preg_match("/^([om])(\d*)/",$fields[0],$matches)) {
			$drugname1 = 'naproxyn';
			$drugname2 = 'naproxyn ';
			if ($matches[1] == 'm') {$drugname1 = 'naproxyn';$drugname2 = 'naproxyn ';}
			array_shift($fields);
			if ($matches[2]) {
