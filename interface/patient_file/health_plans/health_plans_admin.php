...
if ($task == "add" or $task == "update") {
	...
	if (formData('include_lab_show','P',true))
	{
		...
		for ($i = 0; $i < count($include_lab_abnormal_result_history); ++ $i) {
			$check_procedure = sqlQuery("SELECT procedure_type_id FROM procedure_type WHERE name LIKE '".$include_lab_abnormal_result_history[$i]."'");
	...
	if (formData('exclude_lab_show','P',true))
	{
		...
		for ($i = 0; $i < count($exclude_lab_abnormal_result_history); ++ $i) {
			$check_procedure = sqlQuery("SELECT procedure_type_id FROM procedure_type WHERE name LIKE '".$exclude_lab_abnormal_result_history[$i]."'");
    ...
    if ($from_month == "" and $fromage == "" and $to_month=="" and $toage=="") {
        ...
        $where_age = "AND DOB BETWEEN '".$todate."' AND '".$formdate."'";
    }
...
switch($task) {
    case "delete": {
        ...
        for($i = 0; $i < count($delete_id); $i++) {
            $checkdel = sqlQuery("SELECT COUNT(*) AS count FROM health_plan_enrollment WHERE plan_id = '". formDataCore($delete_id[$i]) ."'"); // check if any patients under this plan
            if ($checkdel['count']) {
              $alertmsg = xl("Cannot delete this entry because it\'s being used!");
            } 
            else {
                sqlStatement("DELETE FROM health_plans WHERE plan_id = '". formDataCore($delete_id[$i]) ."'");
                sqlStatement("DELETE FROM health_plan_actions WHERE plan_id = '". formDataCore($delete_id[$i]) ."'");
                sqlStatement("DELETE FROM health_plan_enrollment WHERE plan_id = '". formDataCore($delete_id[$i]) ."'");
                sqlStatement("DELETE FROM patient_reminders WHERE plan_id = '". formDataCore($delete_id[$i]) ."'");
                $getalerts = sqlQuery("SELECT alert_id FROM clinical_alerts WHERE plan_id = '". formDataCore($delete_id[$i]) ."'");
                sqlStatement("DELETE FROM clinical_alerts where plan_id='". formDataCore($delete_id[$i]) ."'");
                sqlStatement("DELETE FROM patient_alerts where alert_id='".formDataCore($getalerts['alert_id'])."'");
            }
        }
        $task = "";
      } break;
    ...
    case "update": {
        ...
        $crow = sqlQuery("SELECT COUNT(*) AS count FROM health_plans WHERE plan_name = '" . $plan_name . "' AND plan_id != '$plan_id'");
        if ($crow['count']) {
            ...
            if (!empty($cpt_include) or !empty($cpt_exclude)) $cpt_where = " AND code_type = 'CPT4'";
            ...
            if ($task == "update") {
                $query = "UPDATE health_plans SET $sql WHERE plan_id = '$plan_id'";
                sqlStatement($query);
                sqlStatement("DELETE FROM health_plan_actions WHERE plan_id = '$plan_id'");
                for ($k=1;$k<=$total_action;$k++) {
                    $sql_action = "";
                    $sql_action = 
                    "plan_id ='" . $plan_id . "', " .
                ...
                while ($rows = sqlFetchArray($result)) {
                      ...
                      if (!empty($icd_include) or !empty($icd_exclude) or !empty($cpt_include) or !empty($cpt_exclude)) {
                        $cres = sqlQuery("SELECT COUNT(*) as count FROM billing WHERE pid='".$rows['pid']."' AND ((1 = 1".$icd_where.") OR (1 = 1".$cpt_where."))");
                        if ($cres['count'] == 0) {
							$get_patient_id = "";
                        }
                      } 
                      if (!empty($ndc_where) or !empty($allergy_where)) {
                        $cres = sqlQuery("SELECT COUNT(*) as count FROM lists WHERE pid='".$rows['pid']."' AND (1 = 1".$ndc_where.") OR (1 = 1".$allergy_where.")");
                        if ($cres['count'] == 0) {
							$get_patient_id = "";
                        }
                      } 
                      if (!empty($patient_history_include) or !empty($patient_history_exclude)) {
					  	if ($patient_history_include) {
							...
							for ($i = 0; $i < count($tmp); ++ $i) {
								...
								$historylike .= str_replace(" ", "_", substr(strtolower($tmp2[1]), strpos($tmp2[1], " ") + 1))." like '$tmp2[0]||".str_replace(" ", "", strtolower($tmp2[1]))."%'";
							}
							if ($historylike)
							{
					  	...
					  	if ($patient_history_exclude) {
							...
							for ($i = 0; $i < count($tmp); ++ $i) {
								...
								$historynotlike .= str_replace(" ", "_", substr(strtolower($tmp2[1]), strpos($tmp2[1], " ") + 1))." like '$tmp2[0]||".str_replace(" ", "", strtolower($tmp2[1]))."%'";
							}
							if ($historynotlike)
							{
                ...
                if ($activation_status=="activate") {
					...
					if (!empty($patientid)) {
						 foreach($patientid as $val) {
							$prow = sqlQuery("SELECT * FROM health_plan_enrollment WHERE patient_id = '$val' AND plan_id = '$plan_id'");
							if ($prow) {
								sqlStatement("UPDATE health_plan_enrollment SET  status = '$status' where patient_id = '$val' AND plan_id = '$plan_id'");
								...
								sqlQuery("delete from patient_reminders WHERE patient_id = '$val' AND plan_id = '$plan_id' AND enroll_id='".$prow['enroll_id']."'");
					...
					sqlStatement("delete from patient_alerts where alert_id='".formDataCore($getalerts['alert_id'])."'");
					sqlStatement("DELETE FROM health_plan_enrollment WHERE plan_id = '$plan_id'");
					sqlStatement("DELETE FROM patient_reminders WHERE plan_id = '$plan_id'");
                }
              } 
            else {
...
function validEntry(f) {
	if (!f.plan_name.value) {
  		alert('<?php xl('No plan name was specified!','e') ?>');
  		return false;
 }
 return true;
}
...
<table><tr><td><font class=title><?php xl('Health Plans','e'); ?></font></td></tr></table>
<?php } else { ?>
<table><tr><td><a href="health_plans_admin.php" target="Main" onClick="top.restoreSession()">
<font class=title><?php xl('Health Plans','e'); ?></font></a></td></tr></table>
<?php }
echo "<br/>";
...
<table border=0 cellpadding=1 cellspacing=1>
    <tr>
        <td class='required'> <?php xl('Plan Name','e'); ?>:</td>
        <td width=10></td>
    ...
    <tr>
        ...
        <td class='text'>
        <div>
        <input type='checkbox' name='include_icd_show' id='include_icd_show' onclick='return divclick(this,"div_include_icd");' <?php echo (!empty($icd_include))?"checked":"";?>/><?php xl('ICD','e'); ?>
        <input type='checkbox' name='include_cpt_show' id='include_cpt_show' onclick='return divclick(this,"div_include_cpt");' <?php echo (!empty($cpt_include))?"checked":"";?>/><?php xl('CPT','e'); ?>
        <input type='checkbox' name='include_ndc_show' id='include_ndc_show' onclick='return divclick(this,"div_include_ndc");' <?php echo (!empty($ndc_include))?"checked":"";?>/><?php xl('Medication','e'); ?>
        <input type='checkbox' name='include_allergy_show' id='include_allergy_show' onclick='return divclick(this,"div_include_allergy");' <?php echo (!empty($allergy_include))?"checked":"";?>/><?php xl('Allergy','e'); ?>
        <input type='checkbox' name='include_pt_history_show' id='include_pt_history_show' onclick='return divclick(this,"div_include_pt_history");' <?php if ($patient_history_include != '') echo " checked";?>/><?php xl('Patient History','e'); ?>
        <input type='checkbox' name='include_lab_show' id='include_lab_show' onclick='return divclick(this,"div_include_lab");' <?php if ($lab_abnormal_result_include != '') echo " checked";?>/><?php xl('Lab Abnormal Result','e'); ?>
        </div>
                ...
                if (!empty($icd_include)) {
                    ...
                    foreach ($icd_in_code as $icd_in_val) {
                        ...
                        while ($coderows = sqlFetchArray($coderesult)) {
                            if ($descs != "") $descs .= "::"; 
                            echo $coderows['code']. " " . $coderows['code_text']. "<br />";
                            $descs .= $coderows['code']. " " . $coderows['code_text'];
                        }
                    }
                }
                else {
                ...
                if (!empty($cpt_include)) {
                    ...
                    foreach ($cpt_in_code as $cpt_in_val) {
                        ...
                        while ($coderows1 = sqlFetchArray($coderesult1)) {
                            if ($descs != "") $descs .= "::";
                            echo $coderows1['code']. " " . $coderows1['code_text']. "<br />";
                            $descs .= $coderows1['code']. " " . $coderows1['code_text'];
                        }
                    }
                }
                else {
				...
				if (!empty($lab_abnormal_result_include)) {
                    ...
                    foreach ($lab_abnormal_result_in_procedure as $lab_abnormal_result_in_val) {
                        ...
                        while ($procedurerows = sqlFetchArray($procedureresult)) {
							$codes[] = $procedurerows['name'];
							$descs[] = $procedurerows['name']. " " . $procedurerows['description'];
                        }
                    }
					$codes = implode("::", $codes);
					$descs = implode("::", $descs);
    ...
    <tr>
        ...
        <td class='text'>
        <div>
        <input type='checkbox' name='exclude_icd_show' id='exclude_icd_show' onclick='return divclick(this,"div_exclude_icd");' <?php echo (!empty($icd_exclude))?'checked':'';?>/><?php xl('ICD','e'); ?>
        <input type='checkbox' name='exclude_cpt_show' id='exclude_cpt_show' onclick='return divclick(this,"div_exclude_cpt");' <?php echo (!empty($cpt_exclude))?'checked':'';?>/><?php xl('CPT','e'); ?>
        <input type='checkbox' name='exclude_ndc_show' id='exclude_ndc_show' onclick='return divclick(this,"div_exclude_ndc");' <?php echo (!empty($ndc_exclude))?'checked':'';?>/><?php xl('Medication','e'); ?>
        <input type='checkbox' name='exclude_allergy_show' id='exclude_allergy_show' onclick='return divclick(this,"div_exclude_allergy");' <?php echo (!empty($allergy_exclude))?'checked':'';?>/><?php xl('Allergy','e'); ?>
        <input type='checkbox' name='exclude_pt_history_show' id='exclude_pt_history_show' onclick='return divclick(this,"div_exclude_pt_history");' <?php if ($patient_history_exclude != '') echo " checked";?>/><?php xl('Patient History','e'); ?>
        <input type='checkbox' name='exclude_lab_show' id='exclude_lab_show' onclick='return divclick(this,"div_exclude_lab");' <?php if ($lab_abnormal_result_exclude != '') echo " checked";?>/><?php xl('Lab Abnormal Result','e'); ?>
        </div>
                ...
                if (!empty($icd_exclude)) {
                    ...
                    foreach ($icd_ex_code as $icd_ex_val) {
                        ...
                        while ($coderows2 = sqlFetchArray($coderesult2)) {
                            if ($descs != "") $descs .= "::";
                            echo $coderows2['code']. " " . $coderows2['code_text']. "<br />";
                            $descs .= $coderows2['code']. " " . $coderows2['code_text'];
                        }
                    }
                }
                else {
            ...
            <tr>
                ...
                if (!empty($cpt_exclude)) {
                    ...
                    foreach ($cpt_ex_code as $cpt_ex_val) {
                        $codequery3 = "SELECT * FROM codes WHERE code_type='1' AND code = '".formDataCore($cpt_ex_val)."'";
                        $coderesult3 = sqlStatement($codequery3);
                        while ($coderows3 = sqlFetchArray($coderesult3)) {
                            if ($descs != "") $descs .= "::";
                            echo $coderows3['code']. " " . $coderows3['code_text']. "<br />";
                            $descs .= $coderows3['code']. " " . $coderows3['code_text'];
                        }
                    }
                }
                else {
...
<?php for ($j = 1; $j<=10; $j++) { ?>
		...
		<table>
            <tr>
                <td class='text' width="125"><?php xl('Target Date','e');?>:</td>
                <td width=10></td>
            ...
            <tr>
                ...
                <td width=10></td>
                <td class='text'><input type="text" size="4" name="reminder_timeframe_<?php echo $j; ?>" value="<?php echo htmlspecialchars(${"reminder_timeframe_$j"}, ENT_QUOTES);?>"> <?php xl('Days','e');?></td>
            </tr>
            <tr>
                <td class='text'><?php xl('Followup Timeframe','e');?>:</td>
                <td width=10></td>
                <td class='text'><input type="text" size="4" name="followup_timeframe_<?php echo $j; ?>" value="<?php echo htmlspecialchars(${"followup_timeframe_$j"}, ENT_QUOTES);?>"> <?php xl('Days','e');?></td>
            </tr>
            <tr>
		...
		<?php for ($k = 1; $k<=10; $k++) { ?>
				...
				<table>
					...
					<tr>
						...
						<td class='text'><?php xl('Reminder Timeframe','e');?>:</td>
						<td width=10></td>
						<td class='text'><input type="text" size="4" name="reminder_timeframe_<?php echo $j;?>-<?php echo $k; ?>" value="<?php echo htmlspecialchars(${"reminder_timeframe_$j-$k"}, ENT_QUOTES);?>"> <?php xl('Days','e');?></td>
					</tr>
					<tr>
						<td width=140></td>
						<td class='text'><?php xl('Followup Timeframe','e');?>:</td>
						<td width=10></td>
						<td class='text'><input type="text" size="4" name="followup_timeframe_<?php echo $j;?>-<?php echo $k; ?>" value="<?php echo htmlspecialchars(${"followup_timeframe_$j-$k"}, ENT_QUOTES);?>"> <?php xl('Days','e');?></td>
					</tr>
					<tr>
						<td width=140></td>
						<td class='text'><?php xl('Completed','e');?>:</td>
...
<?php } ?>
    ...
    <tr>
        <td class='text'><?php xl('Activation Status','e');?>:</td>
        <td width=10></td>
        <td class='text'><input type="radio" name="activation_status" value="activate" checked><?php xl('Activate Plan for All Patients','e');?> <input type="radio" name="activation_status" value="deactivate" <?php echo $activation_status=="deactivate"?'checked':''; ?>><?php xl('Deactivate Plan for All Patients','e');?></td>
    </tr>
    <tr>
        ...
        if ($task == "edit") {
        ?>
        <input type="button" id="update" onClick="submitUpdate();" value="<?php xl('Update','e'); ?>">
        &nbsp;&nbsp;
        <?php 
        }
        else {
        ?>
        <input type="button" id="add_as_new" onClick="submitAdd();" value="<?php xl('Add as New','e'); ?>">
        <?php
        }
        ?>
        &nbsp;&nbsp;
        <input type="button" id="cancel" onClick="submitCancel();" value="<?php xl('Cancel','e'); ?>">
        </td>
    </tr>
</table>
</center>
...
else
{
    ...
    for($i = 0; $i < count($sort); $i++) {
        $sortlink[$i] = "<a href=\"health_plans_admin.php?sortby=$sort[$i]&sortorder=asc\" onclick=\"top.restoreSession()\"><img src=\"../../../images/sortdown.gif\" border=0 alt=\"".xl('Sort Up')."\"></a>";
    }
    for($i = 0; $i < count($sort); $i++) {
        if($sortby == $sort[$i]) {
            switch($sortorder) {
                case "asc"      : $sortlink[$i] = "<a href=\"health_plans_admin.php?sortby=$sortby&sortorder=desc\" onclick=\"top.restoreSession()\"><img src=\"../../../images/sortup.gif\" border=0 alt=\"".xl('Sort Up')."\"></a>"; break;
                case "desc"     : $sortlink[$i] = "<a href=\"health_plans_admin.php?sortby=$sortby&sortorder=asc\" onclick=\"top.restoreSession()\"><img src=\"../../../images/sortdown.gif\" border=0 alt=\"".xl('Sort Down')."\"></a>"; break;
            } break;
        }
    }
    ...
    <input type=hidden name=plan_id value=$plan_id>
        <tr height=\"24\" style=\"background:lightgrey\">
            ...
            <td style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid;\" class=bold>&nbsp;<b>".xl('Plan Name')."</b> $sortlink[0]</td>
            <td width=\"30%\" style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid;\" class=bold>&nbsp;<b>".xl('Category')."</b> $sortlink[1]</td>
            <td width=\"15%\" style=\"border-bottom: 1px #000000 solid; \" class=bold>&nbsp;<b>".xl('Status')."</b> $sortlink[2]</td>
        </tr>";
        ...
        while ($myrow = sqlFetchArray($result)) {
            $category_row = sqlQuery("SELECT title FROM list_options WHERE option_id = '".$myrow["category"]."' LIMIT 1");
            $category_name = trim($category_row['title']);
            $status = ($myrow['activation_status']=="activate"?"Activated":"Deactivated");
            $count++;
            echo "
            <tr id=\"row$count\" style=\"background:white\" height=\"24\">
                <td align=\"center\" style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid;\"><input type=checkbox id=\"check$count\" name=\"delete_id[]\" value=\"".$myrow['plan_id']."\" onclick=\"if(this.checked==true){ selectRow('row$count'); }else{ deselectRow('row$count'); }\"></td>
                <td style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid;\"><table cellspacing=0 cellpadding=0 width=100%><tr><td width=5></td><td class=\"text\"><a href='javascript:submitEdit(". $myrow['plan_id'] .")'>".$myrow['plan_name']."</a></td><td width=5></td></tr></table></td>
    ...
    <table border=0 cellpadding=5 cellspacing=0 width=90%>
        <tr>
            <td class=\"text\"><a href=\"health_plans_admin.php?sortby=$sortby&sortorder=$sortorder&begin=$begin&task=addnew\" onclick=\"top.restoreSession()\">".xl('Add New')."</a> &nbsp; <a href=javascript:confirmDeleteSelected() onclick=\"top.restoreSession()\">".xl('Delete')."</a></td>
