...
function health_plans_enrollment($plan_id, $pid, $enroll_id, $reminderstatus, $type) {
	...
	$planrow = sqlQuery("select plan_name from health_plans where plan_id = '$plan_id'");

	// Get the sender name for the reminder and followup message.
	$patient = sqlQuery("select fname from patient_data where pid='$pid'");

	// Get the sender name for the reminder and followup message.
	$sender = sqlQuery("select sender_name from patient_reminders where reminder_name='Sender Details'");

	// Select the plan action based on plan ID.
	$actionsql = "select * from health_plan_actions where plan_id = '$plan_id' order by action_id";
	...
	switch($type)
	{
		case "admin" : {
			...
			if ($form_action_targetdate) {
				sqlQuery("update health_plan_enrollment set action_date='".implode(",", $form_action_targetdate)."' where enroll_id='$enroll_id'");
			}
		} break;
		case "batch" :
		case "patient" : {
			// Update the action date from the reminder of the enrollment.
			sqlQuery("update health_plan_enrollment set action_date='".implode(",", $form_action_targetdate)."', action_completed ='".implode(",", $form_action_completed)."' where enroll_id='$enroll_id'");
	
			if (formData("form_signup_date") && formData("form_enrollment_status")) {
				// Update the signup date and action date from the reminder of the enrollment.
				sqlQuery("update health_plan_enrollment set signup_date='".formData("form_signup_date")."', status='".formData("form_enrollment_status")."' where enroll_id='$enroll_id'");
			}
		} break;
	}
}
