...
if (!empty($pid) and $task == "") {
	$sql = "SELECT clinical_alerts.plan_id AS plan_id, patient_alerts.alert_id AS alert_id FROM patient_alerts JOIN clinical_alerts ON patient_alerts.alert_id= clinical_alerts.alert_id WHERE patient_alerts.pid = '$pid' AND patient_alerts.status != 'Done'";
...
if (!empty($pid) and !empty($arr_alert_id)) { // Check to see if clinical alerts exist.
	...
	foreach ($arr_alert_id as $alert_id) { // Show each patient alert.
		...
		$cres = sqlQuery("SELECT COUNT(*) as count FROM clinical_alerts, health_plans WHERE clinical_alerts.alert_id='$alert_id' and health_plans.plan_id=clinical_alerts.plan_id and health_plans.activation_status='activate'");
		if ($cres['count']) {
		//$row = sqlQuery("SELECT * FROM patient_alerts WHERE pid = '$pid' AND alert_id = '$alert_id'");
	  	...
	  	if (!empty($all_id))
	   		...
	   		$all_id = $alert_id;
	  	$alertresult = sqlStatement("SELECT * FROM clinical_alerts WHERE alert_id = '$alert_id'");
	  	while ($alertrow = sqlFetchArray($alertresult)) { // get detail from clinical_alerts
	   		...
	   		if ($alertrow['past_due_message'] == "") {
	    		...
	    		$past_due_message = $alertrow['past_due_message'];
	   		}
	   		if ($alertrow['message'] == "") {
	    		$past_due_message = "No message has been set up for this alert. Please talk to your Administrator.";
	   		} else {
	   		...
	   		sqlStatement("UPDATE clinical_alerts SET activated = '".($alertrow['activated'] + 2)."' WHERE alert_id = '".$alert_id."'");
	 ?>
	 <tr>
	  <td height="5">
	   <br/>
	 ...
	 <tr>
	  <td class='text' align='left'><b><?php echo xl('Alert'). " #".$total_alerts; ?>:</b>
	  <td width=10></td>
	  <td class='text'><?php echo "<font color=\"".$alert_color."\">".$alertrow['alert_name']."</font>"; ?></td>
	 </tr>
	 <tr>
	 ...
	 <tr>
	  ...
	  <?php
	  		if (!empty($plan_id)) { // Check to see if the health plan exists.
	    		$sql = "SELECT plan_name FROM health_plans WHERE plan_id = '".$alertrow['plan_id']."'";
	    		$presult = sqlQuery($sql);
	    		$plan_name = $presult['plan_name'];
	    		if (empty($check['count']) and $check['count'] <= 0) { //  check enroll part. if not enrolled
	  ?>
	     			<input type='checkbox' name="track[<?php echo $alert_id?>]" value="<?php echo $plan_id; ?>" id="track_<?php echo $alert_id ?>"/><font color="<?php echo $alert_color; ?>"> <?php echo xl('Discussed')." ".$plan_name." ".xl('Health Plan');?></font><br />
	    <?php
	    		} else { //if enrolled
	    ?>
	     			<input type='checkbox' name="track[<?php echo $alert_id?>]" value="<?php echo $plan_id; ?>" id="track_<?php echo $alert_id ?>"/><font color="<?php echo $alert_color; ?>"> <?php echo xl('Reviewed the status of')." ".$plan_name." ".xl('Health Plan');?></font><br />
	    <?php
	    		}
	 ...
	 <tr>
	  <td class='text' valign="top" align='left'><b><?php xl('Other','e'); ?>:</b></td>
	  <td width=10></td>
	  <td class='text'>
	   <input type='checkbox' name="track_education[<?php echo $alert_id;?>]" id='track_education_<?php echo $alert_id;?>' value="<?php echo $alert_id;?>"/><font color="<?php echo $alert_color; ?>"> <?php xl('Provided Patient Education','e')?></font>
	  </td>
	 </tr>
	 <?php
	   $total_alerts++;
	  ...
	  echo "<input type=\"hidden\" name=\"id\" value=\"$all_id\">";
	} // end each patient alerts
	 ?>
	 <tr>
	  <td class='text' align='center' colspan="3"><br>
	    <input type="button" value="<?php xl('Save','e'); ?>" onClick="submitclose()">&nbsp;&nbsp;
