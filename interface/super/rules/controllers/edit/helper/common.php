...
<select data-grp-tgt="<?php echo out( $args['target'] ); ?>" type="dropdown" name="<?php echo out( $args['name'] ); ?>" id="<?php echo out( $args['id'] ); ?>">
    <option id="" value="">--<?php echo xl("Select"); ?>--</option>
    <?php foreach( TimeUnit::values() as $unit ) { ?>
    <option id="<?php echo out( $unit->code ); ?>" value="<?php echo out( $unit->code ); ?>" <?php echo $args['value'] == $unit ? "SELECTED" : "" ?>>
        <?php echo out( xl( $unit->lbl ) ); ?>
    </option>
    <?php } ?>
</select>
<?php } ?>
